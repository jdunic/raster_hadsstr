...
getSSTAnnualRasters <- function(hadsst_raster, years = 1969:2011) {
    # Need to set up list of years and a corresponding index to allow renaming
    # of the raster@z value.
    seqlen <- seq_along(years)
    x <- c(years, seqlen)
    x <- matrix(x, ncol = 2)
    year_list <- lapply(seqlen, function(i) x[i, ])

    lapply(year_list, function(elem) {
    browser()
        yearIDx <- which(chron::years(hadsst_raster@z$Date) == elem[[1]])
...
x <- c(years, y)
x <- matrix(x, ncol = 2)
lapply(seqlen, function(i) x[i, ])

lapply(seq_len(ncol(x)), function(i) x[i,])

annual_rast_2011 <- getSSTAnnualRasters(hadsst_raster = b, years = 2010:2011)
```
